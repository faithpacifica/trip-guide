...
const Shadow = styled.div`
  background-image: linear-gradient(
    175.54deg,
    rgba(2, 0, 14, 0.13) 9.38%,
    rgba(42, 34, 101, 0) 47.26%,
    rgba(15, 11, 44, 0.6) 92.77%
  );
  z-index: 5px;
  position: absolute;
  left: 0;
...
const TopTourCard = ({ tourobj }) => {
  ...
  return (
    <TourTopCard className="top-tour__card">
      <TourTopImg
        src={`/assets/img/${tourobj.photo}`}
        alt={tourobj.name}
        width="370"
        height="495"
      />
      <Shadow></Shadow>
      <button type="button">{tourobj.name}</button>
      <TourTopContent>
        <TourTopTitle>{tourobj.country}</TourTopTitle>
        <TourTopText>
          <span> {tourobj.place_count} </span> {t('popularPlaces')}
        </TourTopText>
      </TourTopContent>
    </TourTopCard>
  );
